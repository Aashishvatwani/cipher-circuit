...
export default function Round1() {
  ...
  useEffect(() => {
    ...
    if (!teamId) {
      // For demo, set a mock teamId
      localStorage.setItem('teamId', 'team-001');
    }
    fetchTeamAssignment(localStorage.getItem('teamId') || 'team-001');
  }, []);
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (JSON.stringify(keyBits.map(Number)) !== JSON.stringify(correctKey)) {
      setError('Incorrect key. Check your XOR calculations.');
      setSubmitting(false);
      return;
    }
    ...
    setSuccess(true);
    setTimeout(() => {
      // Navigate to round 2 or show success
      window.location.href = '/round2/encrypt';
    }, 1800);
  };
                  ...
                  {success && (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-lime-400/70 text-xs">Advancing to Round 2...</p>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <XORLogicCard 
                    ...
                    label={label} 
                    formula={formula} 
                    result={correctKey ? correctKey[idx] : null}
                    index={idx}
                  />
                ))}
              </div>
        ...
        >
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-6 text-white/40 text-sm">
              ...
              <span className="font-mono">Round 1 of 3</span>
            </div>
          </div>
        </motion.footer>
      </div>
